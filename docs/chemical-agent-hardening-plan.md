...
| 6 — Cost caching | **IN PROGRESS — Slice A + B DONE (uncommitted)** 2026-07-05 | User chose "metering first, then decide" (measure token spend + repeat rates before picking a cache tier). **Slice A shipped 2026-07-05** (committed `c0bd1d6a` alongside Phase 5): migration **0031** `chat_logs.input_tokens`/`output_tokens INTEGER` (additive/idempotent, **applied dark** to control DB `tticllabbbqwnhsmggfo`); `run_agent_loop(..., usage_out=dict)` accumulator; `log_chat_to_db(..., input_tokens, output_tokens)`; `services/session_bi.build_token_metrics()` + `token_metrics` block; `SessionBiPanel.tsx` "Agent cost" row. **Item 1 (explicit Gemini context caching) INVESTIGATED AND KILLED 2026-07-05** — the pre-build audit's feasibility claim was wrong on two counts, both verified directly (not trusted from docs/subagent reports): (a) `langchain-google-genai` 4.2.1 DOES support `cached_content` publicly (`ChatGoogleGenerativeAI(cached_content=cache.name)`, cache created via `google.genai.Client().caches.create()` — no LangChain bypass needed, contrary to an initial exploration finding); BUT (b) Gemini's **explicit** caching requires a **32,768-token minimum** to create a cache (confirmed against Google's own docs), and our static system-prompt prefix (platform rules + chemical agent directive + tool schemas) measures only **~2,286 tokens** — 14x too small. Padding the prefix to qualify would cost more tokens than the 90% discount would save. **Item 1 is not viable at this prompt size and is dropped from scope** (revisit only if the static prefix ever grows past ~33K tokens — e.g. a large embedded catalog — which is not currently planned). **Slice B shipped 2026-07-05 (uncommitted):** since Gemini 2.x applies **implicit** caching automatically (min ~1,024 tokens, our 2,286-token prefix already clears it, zero code needed for the caching itself) but we had no visibility into whether it fires, added migration **0032** `chat_logs.cached_tokens INTEGER` (additive/idempotent, not yet applied to control DB); `_accumulate_usage` now also reads `usage_metadata.input_token_details.cache_read` (the field LangChain already surfaced per-call, previously uncaptured); threaded through `log_chat_to_db` + the vertical logging call site; `build_token_metrics` gains `cached_tokens` + `prompt_cache_hit_rate` (cached_tok / input_tok); `SessionBiPanel.tsx` gains a "Prompt cache reads" MetricCard. Tests: +2 `test_agent.py` (cache_read capture + absent/zero case), +2 `test_session_bi.py`. Backend 1386 pass / 125 skip; frontend 352 pass (1 pre-existing unrelated `insights-panels` fail); tsc clean. **NEXT = let real `prompt_cache_hit_rate` data accrue in the panel; if implicit caching is already hitting well, Phase 6 may need nothing further beyond metering. If the rate is low/zero, investigate why before considering item 3 (guarded answer cache) — item 2 (tool-result cache) remains flagged marginal.** |
...
1. ~~**Gemini explicit context caching**~~ **DROPPED 2026-07-05 — not viable.**
   Originally scoped as caching the static prompt prefix (platform rules +
   persona + agent directive + tool schemas), keyed per company, TTL ~1h. Verified
   during implementation: Gemini's *explicit* caching requires a 32,768-token
   minimum to create a cache; our static prefix measures ~2,286 tokens (14x too
   small). Padding the prefix to qualify would cost more tokens than the discount
   saves. Gemini 2.x's *implicit* caching (automatic, min ~1,024 tokens, our
   prefix already clears it) covers this for free — see Slice B in the ledger.
   Revisit explicit caching only if the static prefix ever grows past ~33K tokens.
...
4. Metering: log cache hit-rate + per-turn token counts into the existing
   ...
   **Slice A** (input/output tokens) + **Slice B** (Gemini implicit-cache-read
   tokens, `prompt_cache_hit_rate`) shipped 2026-07-05 — see ledger.
