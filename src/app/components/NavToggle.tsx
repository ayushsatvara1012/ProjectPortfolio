...
export default function NavToggle() {
  ...
  const showOnPaths = ['/dashboard', '/dashboard/register', '/dashboard/settings/admin'];
  const shouldShow = pathname ? showOnPaths.includes(pathname) : false;

  if (!shouldShow) return null;

  const navItems = [
    { name: 'Dashboard', path: '/dashboard', icon: LayoutDashboard },
    { name: 'Register', path: '/dashboard/register', icon: Key },
    ...(userRole === 'SUPER_ADMIN'
      ? [{ name: 'Platform', path: '/dashboard/settings/admin', icon: ShieldCheck }]
      : []),
  ];
        ...
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              ...
              <Icon className="relative z-10 w-4 h-4 sm:w-3.5 sm:h-3.5" />
