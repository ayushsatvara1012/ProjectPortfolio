...
export default function DemoMyBotsPage() {
    ...
    return (
        <div className="flex flex-col min-h-full bg-slate-50 dark:bg-slate-900 transition-all duration-500 relative overflow-hidden">
            <div className="absolute inset-0 bg-cover bg-center opacity-100" style={{ backgroundImage: "url('/nature.webp')" }} />
            <div className="absolute inset-0 bg-white/40 dark:bg-slate-950/70 backdrop-blur-[2px]" />

            <div className="relative z-10">
                <div className="bg-white/70 dark:bg-slate-950/70 backdrop-blur-md px-8 py-6 border-b border-gray-100 dark:border-slate-800 flex justify-between items-center">
                    <div>
                        <h1 className="text-2xl font-bold">My Bots</h1>
                        <p className="text-sm text-slate-500">Manage your AI assistants (Demo Mode).</p>
                    </div>
                    <div className="flex items-center gap-4">
                         <span className={`px-3 py-1 text-[10px] uppercase font-bold border ${speedInfo.cls}`}>{speedInfo.label} Speed</span>
                         <span className="text-sm font-bold text-slate-500">1 / 1 bots</span>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-px bg-gray-200/30 dark:bg-slate-800/30 border-b border-gray-100 dark:border-slate-800 transition-colors">
                    {[
                        ...
                        <div key={i} className="bg-white/50 dark:bg-slate-950/70 backdrop-blur-md px-4 py-3 sm:px-6 sm:py-4 transition-colors">
                            <p className="text-[10px] uppercase tracking-widest font-bold text-slate-400 dark:text-slate-500 mb-0.5">{s.label}</p>
                            <p className="text-lg font-semibold text-slate-900 dark:text-slate-200">{s.value}</p>
                        </div>
                    ))}
                </div>
                ...
                <div className="p-8">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        ...
                        >
                            ...
                            <div className="p-5 flex flex-col flex-1 gap-3">
                                <div className="flex items-start justify-between">
                                    <div>
                                        <h3 className="text-md font-bold text-slate-900 dark:text-slate-200">{bot.bot_name}</h3>
                                        <p className="text-xs uppercase tracking-widest font-bold text-slate-400 dark:text-slate-500 mt-0.5">{bot.company_name}</p>
                                    </div>
                                ...
                                <a href={bot.allowed_origin} target="_blank" rel="noopener noreferrer"
                                    className="text-sm tracking-wide font-medium text-blue-600 dark:text-slate-500 truncate flex items-center gap-2">
                                    <span className="material-symbols-outlined text-[16px]">link</span> {bot.allowed_origin}
                                </a>
                                <div className="flex items-center gap-2 px-3 py-2 border text-[10px] uppercase tracking-widest font-bold
                                ...
                                <div>
                                    <div className="flex justify-between text-[10px] uppercase tracking-widest font-bold text-slate-400 dark:text-slate-500 mb-1">
                                        <span>Usage</span>
                                ...
                                <div className="flex gap-2 mt-auto pt-3 border-t border-gray-100 dark:border-slate-800">
                                    <button onClick={() => router.push('/demo/train')}
                                        className="flex-1 py-2 text-[10px] uppercase tracking-widest font-bold bg-slate-900 dark:bg-blue-600 text-white hover:bg-slate-800 dark:hover:bg-blue-500 transition-colors flex items-center justify-center gap-1 cursor-pointer">
                                        <span className="material-symbols-outlined text-[12px]">psychology</span> Train
                                    </button>
                                    <button onClick={() => router.push('/demo/customize')}
                                        className="flex-1 py-2 text-[10px] uppercase tracking-widest font-bold border border-gray-100 dark:border-slate-700 text-slate-700 dark:text-slate-300 hover:bg-gray-50 dark:hover:bg-slate-800 transition-colors flex items-center justify-center gap-1 cursor-pointer">
                                        <span className="material-symbols-outlined text-[12px]">settings</span> Settings
                                    </button>
                                    <button onClick={() => router.push('/demo/chat')}
                        ...
                        <motion.div layout
                            ...
                            <p className="text-[10px] uppercase tracking-widest font-bold text-slate-300 dark:text-slate-600 text-center">Bot Limit Reached</p>
                            <Link href="/sign-up"
                                className="mt-3 text-[10px] uppercase tracking-widest font-bold text-blue-600 dark:text-blue-400 hover:underline">
                                Sign Up for More →
                            </Link>
                        </motion.div>
                    </div>
