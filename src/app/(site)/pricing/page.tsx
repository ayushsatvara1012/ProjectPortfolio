          ...
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-px bg-gray-100 dark:bg-slate-800">
              ...
              <div className="bg-white dark:bg-slate-950 p-6 md:p-8 flex flex-col gap-4 transition-colors duration-500">
                ...
                <a
                  href="mailto:ayushsatvara2002@gmail.com?subject=Custom%20Plan%20Inquiry&body=Hi%20Ayush%2C%0A%0AI'm%20interested%20in%20a%20custom%20plan.%0A%0AOrganisation%3A%0AExpected%20bots%3A%0AKey%20features%20needed%3A%0A"
          ...
          <div className="hidden md:block overflow-x-auto">
            ...
            >
              <div className="grid grid-cols-5 gap-px bg-gray-200 dark:bg-slate-700 mb-px">
                ...
                {PLANS.map(p => (
                  <div key={p.id} className={`${accentMap[p.accent as keyof typeof accentMap].colBg} px-4 py-4 border-t-2 ${accentMap[p.accent as keyof typeof accentMap].top} transition-colors duration-500`}>
                    ...
                    {p.id === 'BASIC' ? (
                      ...
                      <p className="text-lg font-display font-black text-slate-800 dark:text-slate-200 mt-0.5">
                        {formatPrice(PRICE_MATRIX[p.id as keyof typeof PRICE_MATRIX][currency])}
            ...
            {PLANS.map((plan, i) => {
              ...
              return (
                ...
                >
                  <div className="px-5 py-4 border-b border-gray-100 dark:border-slate-800 flex items-center justify-between">
                    ...
                    {plan.id === 'BASIC' ? (
                      ...
                      <span className="text-md font-display font-bold text-slate-700 dark:text-slate-300">
                        {`${formatPrice(PRICE_MATRIX[plan.id as keyof typeof PRICE_MATRIX][currency])}/mo`}
                      </span>
                    )}
                  </div>
                  {COMPARISON_FEATURES.map(section =>
                    section.rows.map(row => {
                      const val = row[plan.id.toLowerCase() as keyof typeof row] as string | boolean;
                      return (
                  ...
                  <div className="p-4">
                    <Link href="/dashboard/pricing" className={`w-full py-3 text-xs font-display font-bold uppercase tracking-widest flex items-center justify-center gap-2 active:scale-[0.98] transition-all ${a.btn}`}>
                      {plan.id === 'BASIC' ? 'Start Free' : `Get ${plan.name}`}
                      <ArrowRight size={12} />
                    </Link>
                  </div>
