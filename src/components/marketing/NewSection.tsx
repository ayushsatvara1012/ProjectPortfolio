...
const NewSection = () => {
  ...
  const [ragOpacity, setRagOpacity] = useState(0);
    ...
    <section ref={sectionRef} className="relative w-full min-h-screen flex items-center">
      <div className="w-full max-w-7xl mx-auto px-6 md:px-12 lg:px-20 grid grid-cols-1 lg:grid-cols-[40%_60%] gap-12 py-20 lg:py-28">
        ...
        <div className="hidden lg:flex items-center justify-center">
          <span
            className="font-google font-medium text-[8rem] leading-none tracking-tighter text-slate-900 dark:text-white select-none pointer-events-none"
            style={{ opacity: ragOpacity * 0.15, transition: 'opacity 0.4s ease-out' }}
          >
            RAG
          </span>
        </div>
        ...
        <div className="flex flex-col justify-center gap-8">
          <div>
            <h2 className="text-3xl sm:text-4xl font-google font-medium tracking-tight text-slate-900 dark:text-white leading-tight mb-4">
              Smarter than off-the-shelf chatbots
            </h2>
            <p className="text-lg font-google font-medium text-slate-500 dark:text-slate-400 leading-relaxed max-w-md">
            ...
            {features.map((f, idx) => (
              <li key={idx} className="flex items-start gap-4">
                ...
                <div className="flex flex-col gap-1 min-w-0">
                  <p className="font-google font-medium text-lg text-slate-900 dark:text-white leading-snug">
                    {f.title}
                  </p>
