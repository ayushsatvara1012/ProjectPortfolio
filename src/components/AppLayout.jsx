...
const SidebarItem = ({ label, icon: iconName, path, onClick, expanded }) => (
    <NavLink
        to={path}
        onClick={onClick}
        title={!expanded ? label : undefined}
        className={({ isActive }) =>
            `flex items-center gap-3 px-4 py-2.5 text-md font-display transition-all min-h-[44px] border-l-2 w-full overflow-hidden ${isActive
    ...
    >
        {({ isActive }) => (
            <>
                ...
                <span className={`flex-1 truncate transition-all duration-200 ${expanded ? 'opacity-100 w-auto' : 'opacity-0 w-0 pointer-events-none'}`}>{label}</span>
            </>
        )}
    </NavLink>
);
...
// ── Sidebar content ───────────────────────────────────────────────────────────
const SidebarContent = ({ user, onClose, expanded = true }) => {
    ...
    // Collapse settings sub-items when sidebar collapses
            ...
            <nav className="flex-1 overflow-y-auto py-2">
                ...
                <div>
                    ...
                    >
                        ...
                        <span className={`flex-1 text-left transition-all duration-200 ${expanded ? 'opacity-100 w-auto' : 'opacity-0 w-0 pointer-events-none'}`}>Settings</span>
