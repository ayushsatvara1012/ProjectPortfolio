  ...
  return (
    <>
      <header className="fixed top-0 w-full z-50 bg-white border-b border-gray-200 h-16">
        <div className="max-w-7xl mx-auto h-full flex items-center justify-between divide-x divide-gray-200 border-x border-gray-200">
          ...
          <div className="hidden md:flex flex-1 items-center gap-4 lg:gap-8 xl:gap-10 px-6 lg:px-10 h-full">
            {navLinks.map((link) => (
              <div key={`nav-desk-${link.id || link.name}`} className="relative h-full flex items-center" ref={link.dropdown ? dropdownRef : null}>
                {link.dropdown ? (
                  <button
                    onMouseEnter={() => setIsServicesOpen(true)}
                    onClick={() => setIsServicesOpen(!isServicesOpen)}
                    className="text-slate-500 hover:text-slate-900 transition-colors uppercase font-black text-[10px] tracking-[0.25em] h-full flex items-center gap-2 group"
                  >
                    {link.name}
                    ...
                    >
                      {services.map((service, idx) => (
                        ...
                        >
                          <div className="w-10 h-10 border border-gray-100 flex items-center justify-center text-slate-400 group-hover/item:border-indigo-200 group-hover/item:text-indigo-600 transition-all">
                            {service.icon}
                          </div>
                          <div className="text-left">
                            <h4 className="text-[10px] font-black uppercase text-slate-900 tracking-tighter mb-1 leading-none">{service.title}</h4>
                            <p className="text-[9px] text-slate-400 font-mono uppercase tracking-widest">{service.desc}</p>
                          </div>
                        </a>
                      ))}
                    </div>
                  ...
                  <a
                    ...
                    className="text-slate-500 hover:text-slate-900 transition-colors uppercase font-black text-[10px] tracking-[0.25em] py-2 relative group"
                  >
                    {link.name}
          ...
          <div className="hidden md:flex items-center gap-px bg-gray-200 h-full overflow-hidden shrink-0">
            <SignedOut>
              <div className="h-full bg-white flex items-center px-2 lg:px-4">
                <SignInButton mode="modal">
                  <button className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 hover:text-slate-900 transition-colors px-4 py-3">
                    Login
                  </button>
                </SignInButton>
              </div>
              <div className="h-full bg-white flex items-center">
                <SignUpButton mode="modal">
                  <button className="bg-slate-900 text-white text-[10px] font-black uppercase tracking-[0.25em] px-4 lg:px-6 xl:px-8 py-5 h-full hover:bg-indigo-600 transition-all rounded-none shrink-0">
                    Get_Started
                  </button>
                </SignUpButton>
              </div>
            ...
            <SignedIn>
              <div className="h-full bg-white flex items-center px-3 lg:px-6 gap-6">
                <Link
                  to="/app/pricing"
                  className="text-[10px] font-black uppercase tracking-[0.25em] text-slate-900 hover:text-indigo-600 transition-colors"
                >
                  Dashboard
                </Link>
                <div className="h-10 w-10 border border-gray-200 group/user p-0 flex items-center justify-center">
                  <UserButton
                    afterSignOutUrl="/"
                    appearance={{
                      elements: {
          ...
          <div className="flex items-center md:hidden h-full gap-px bg-gray-200">
            <SignedIn>
      ...
      <div
        className={`fixed top-16 left-0 w-full h-[calc(100vh-64px)] z-40 bg-white border-b border-gray-200 transition-all duration-500 ease-in-out md:hidden flex flex-col overflow-hidden ${
      ...
      >
        <div className="flex-1 flex flex-col h-full">
          <div className="flex flex-col bg-white">
            {navLinks.map((link) => (
              <a
                ...
                href={link.href}
                onClick={(e) => handleLinkClick(e, link.href)}
                className="w-full border-b border-gray-100 px-8 py-5 sm:py-6 flex items-center justify-between text-[11px] font-black uppercase tracking-[0.3em] text-slate-600 hover:text-indigo-600 hover:bg-slate-50 transition-colors"
              >
                <span>{link.name}</span>
          ...
          <div className="p-6 sm:p-8 bg-white mt-auto border-t border-gray-200 space-y-4">
            <SignedOut>
              <SignUpButton mode="modal">
                <button className="w-full bg-slate-900 text-white py-6 font-black text-[12px] uppercase tracking-[0.3em] hover:bg-slate-800 transition-all rounded-none mb-3">
                  Start_Free_Trial
                </button>
              </SignUpButton>
              <SignInButton mode="modal">
                  <button className="w-full border border-gray-200 text-slate-900 py-6 font-black text-[10px] uppercase tracking-[0.2em] rounded-none">
                    Login_to_System
                  </button>
              </SignInButton>
            </SignedOut>
            ...
            <SignedIn>
                <Link
                  ...
                  className="w-full bg-slate-900 text-white py-6 font-black text-[12px] uppercase tracking-[0.3em] hover:bg-slate-800 transition-all rounded-none block text-center"
                >
                  SYSTEM_DASHBOARD
                </Link>
            </SignedIn>

            <div className="pt-8 flex items-center justify-between text-slate-400">
