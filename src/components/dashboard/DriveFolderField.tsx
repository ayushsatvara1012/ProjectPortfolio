...
/**
 * The owner points the bot at a Drive folder. This is the whole configuration:
 * no naming pattern, no per-client regex. Filenames are tokenized server-side, so
 * any convention works.
 *
 * Generalized from `CoaFolderField` (spec-finder-plan Phase 2, D4) so a second,
 * independent folder needs a config object rather than a second copy of this file.
 * Which library it configures decides the label, the help text and the endpoint -
 * and nothing else, because pasting, validating, testing and reporting a Drive
 * folder is the same job either way.
 *
 * Test Connection walks the folder for real and reports counts rather than a tick,
 * because H2's failure mode - a Shared Drive folder returning zero files with HTTP
 * 200 - is indistinguishable from an empty folder unless "connected, 0 files" is a
 * visible outcome of its own. It is also the ONE action that forces a fresh walk:
 * the panel below reads the cache this populates, so there is no second refresh
 * button doing the same job.
 */

// Mirrors DRIVE_FOLDER_ID_RE in packs/overrides.py. The backend re-validates and is
...
export function isDriveFolderInvalid(raw: string): boolean {
...
export type DriveFolderFieldProps = {
  value: string;
...
export default function DriveFolderField({
  library, value, onChange, inputCls, labelCls, helpCls, botId, authFetch, savedValue,
}: DriveFolderFieldProps & { library: DriveLibraryConfig }) {
  const invalid = isDriveFolderInvalid(value);
  const folderId = extractDriveFolderId(value);
  // The panel describes the folder the SEARCH uses, which is the saved one. Showing
  // 1,781 healthy documents under a freshly pasted, unsaved link would read as
  ...
  const testConnection = async () => {
    ...
    setResult(null);
    try {
      const res = await authFetch(`/api/companies/${botId}/${library.id}/test-connection`, {
        method: 'POST',
      });
      setResult({
        // "Connected, 0 files" is NOT a success (H2) - a green tick here would
        // ship a bot that says "nothing on file" forever.
        ok: (res?.indexed ?? 0) > 0,
  ...
  return (
    <div className="mt-5 pt-5 border-t border-slate-100 dark:border-slate-800">
      <label className={labelCls} htmlFor={library.fieldId}>
        {library.label}
      </label>
      <input
        id={library.fieldId}
        ...
        aria-describedby={`${library.fieldId}-hint`}
      />
      <p
        id={`${library.fieldId}-hint`}
        className={`${helpCls} ${invalid ? '!text-red-500 dark:!text-red-400' : ''}`}
      >
        {invalid ? library.invalidHelp : library.help}
      </p>
      {folderId && (
        <a
