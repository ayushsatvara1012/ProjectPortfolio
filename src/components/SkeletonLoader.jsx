...
export const AppPageSkeleton = ({ pct: externalPct }) => {
    ...
    return (
        <div className="flex-1 flex items-center justify-center min-h-[50vh] p-8 transition-all duration-500">
            <style>{`
                @keyframes eyeSlide {
                ...
                .brand-wrap { animation: brandFadeIn 0.8s cubic-bezier(0.2, 0.8, 0.2, 1) both; }
                .brand-label { animation: labelPulse 2s ease-in-out infinite; }
            `}</style>
            ...
            <div className="brand-wrap flex flex-col items-center">
                ...
                <div className="w-72 h-3 bg-slate-100 dark:bg-slate-900 rounded-full overflow-hidden mb-6 relative">
                    <div
                        className="h-full rounded-full transition-all duration-300 ease-out shadow-[0_0_20px_rgba(15,32,96,0.3)]"
                        style={{ width: `${pct.toFixed(1)}%`, background: 'linear-gradient(90deg, #0F2060 0%, #5730F5 100%)' }}
                    />
                </div>

                {done ? (
                    <span className="text-xl font-bold font-google text-slate-900 dark:text-slate-100 tracking-tight">
                        Dashboard Ready
                    </span>
                ) : (
                    <span className="brand-label text-xl font-bold font-google text-slate-400 dark:text-slate-500 tracking-tight">
