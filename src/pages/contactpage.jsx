        ...
        <div className="max-w-7xl mx-auto px-4 relative z-10">
          ...
          <div className="my-10 text-center sm:mb-10 sm:mt-0 lg:mb-10 lg:mt-0 lg:text-left">
            <div className="inline-flex items-center gap-2 px-2 py-1 rounded-none bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 mb-4">
              ...
              <span className="text-[9px] font-bold uppercase tracking-[0.2em] text-slate-500 font-mono">Sapybase_v2.0</span>
            </div>
          ...
          <div className="flex flex-col-reverse lg:grid lg:grid-cols-2 gap-8 lg:gap-16 items-start">
            ...
            <div className="w-full space-y-6">
              <div className="space-y-3">
                <h2 className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-[0.2em] text-center lg:text-left">Frequently Asked Questions</h2>
                {faqs.map((faq, i) => (
                  <div key={i} className="rounded-none border border-slate-200 dark:border-slate-800 bg-slate-50/40 dark:bg-slate-900 overflow-hidden transition-all">
                    ...
                    <div className={`px-5 transition-all duration-300 ease-in-out ${activeFaq === i ? 'pb-5 max-h-32 opacity-100' : 'max-h-0 opacity-0'}`}>
                      <p className="text-slate-500 dark:text-slate-400 text-xs leading-relaxed border-l-2 border-slate-200 dark:border-slate-800 pl-4">{faq.a}</p>
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div className="grid grid-cols-1">
                <div type='button' onClick={showError} className="p-4 rounded-none bg-emerald-50 dark:bg-emerald-900/20 border border-dashed border-emerald-200 dark:border-emerald-800/40 flex flex-row items-center justify-center text-center gap-2 hover:bg-emerald-100 dark:hover:bg-emerald-900/40 transition-colors text-emerald-600 dark:text-emerald-400 cursor-pointer">
                  ...
                  <span className="text-[10px] font-bold uppercase tracking-[0.2em]">Track Your Project Status</span>
                </div>
              </div>
            </div>
            ...
            <div className="w-full relative">
              <div className="relative bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-none p-6 md:p-8">
                <div className="flex items-center gap-3 mb-6">
                  ...
                  <h3 className="text-lg font-bold text-slate-900 dark:text-slate-200 tracking-tight">Project Briefing</h3>
                </div>

                <form className="space-y-6">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div className="space-y-1">
                      <label className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-[0.2em] ml-1">Your Identity</label>
                    ...
                    <div className="space-y-1">
                      <label className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-[0.2em] ml-1">Channel</label>
                  ...
                  <div className="space-y-1">
                    <label className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-[0.2em] ml-1">Architecture Overview</label>
                  ...
                  <button onClick={showDev} type='button' className="w-full bg-slate-900 dark:bg-indigo-600 hover:bg-slate-800 dark:hover:bg-indigo-500 text-white px-8 py-5 rounded-none font-bold tracking-[0.2em] uppercase text-[10px] flex items-center justify-center gap-2 transition-colors group">
